...
// CHANGED: Renamed prop 'document' to 'file' to avoid shadowing global 'document'
export default function PreviewModal({ document: file, allDocs, onSelectDoc, onClose }) {
  
  // FIXED: Hook called unconditionally at the top level
  useEffect(() => {
    // Access global document object safely
  ...
  const handleDownload = () => {
    const url = `${API_URL}/documents/${file.id}/download`;
            ...
            <div>
                <div className="flex items-center gap-2 mb-2">
                    <span className="px-2.5 py-0.5 rounded-md bg-indigo-50 text-indigo-700 text-xs font-bold uppercase tracking-wide">
                        {file.ai_journal || "Research Paper"}
                    </span>
                    <span className="text-gray-400 text-sm">• {file.ai_date_created}</span>
                </div>
                <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 leading-tight">
                    {file.title}
                </h2>
                <p className="text-gray-500 font-medium mt-1">
                    {Array.isArray(file.ai_authors) ? file.ai_authors.join(', ') : file.ai_authors}
                </p>
            </div>
            <button 
                onClick={onClose}
        ...
        <div className="flex-grow overflow-y-auto p-8 space-y-8 bg-gray-50/50">
            ...
            <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                ...
                <p className="text-gray-700 leading-relaxed text-lg">
                    {file.ai_abstract || "No abstract available."}
                </p>
            </section>
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-2">
                    {file.ai_keywords && (typeof file.ai_keywords === 'string' ? JSON.parse(file.ai_keywords) : file.ai_keywords).map((k, i) => (
