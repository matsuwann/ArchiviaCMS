...
export default function AdminUserManagement() {
  ...
  const handleArchive = async (userId) => {
    ...
    // Determine flow based on Super Admin status
    if (adminUser.is_super_admin) {
        if (!window.confirm("Are you sure? This user will be deactivated immediately.")) return;
        try {
            await adminDeleteUser(userId); // Direct deactivation
        ...
        // Regular Admin Flow
        const reason = window.prompt("Enter reason for archiving this user (Required):");
        if (reason === null) return;
        if (!reason.trim()) return toast.error("Reason is required to submit a request.");

        try {
            await adminDeleteUser(userId, { reason }); // Send request with reason
                    ...
                    <tr key={user.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        ...
                        <button
                          ...
                          className={`ml-2 py-1 px-3 text-white text-sm font-semibold rounded-lg shadow-md ${isPending ? 'bg-gray-400 cursor-not-allowed' : 'bg-orange-500 hover:bg-orange-600'}`}
                          disabled={user.id === adminUser.userId || isPending}
                        >
